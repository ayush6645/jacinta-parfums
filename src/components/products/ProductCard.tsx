...
export default function ProductCard({ product }: { product: any, key?: string | number }) {
  ...
  const displayImage = product.images?.[0]?.url || product.image || "/Images/placeholder.jpg";
  const displayPrice = product.base_price || product.price || 0;
  const displayCategory = product.category?.name || product.category || "Signature";
    ...
    >
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="text-xl font-serif text-luxury-white">₹{displayPrice}</span>
        </div>
